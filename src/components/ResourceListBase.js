...
export default {
  ...
  computed: {
    ...
    __filteredResources () {
      ...
      if (this.search_) {
        resources = resources.filter(this.__makeSearchFilter(this.search_))
      }
    ...
    __groups () {
      ...
      if (sortItem && sortItem.groups) {
        if (typeof sortItem.groups === 'function') { groups = sortItem.groups(this.__filteredResources) } else { groups = sortItem.groups }
      } else {
        groups = this.groups
      }
  ...
  methods: {
    ...
    __makeSearchFilter (search) {
      ...
      var reFilter = new RegExp(search, 'i')
      return r => {
        return reFilter.test(r.name()) || reFilter.test(r.type()) || reFilter.test(r.description()) || reFilter.test(r.attr('location'))
      }
    },
