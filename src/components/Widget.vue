<template>
  <div class="widget column" :class="{'widget-err': __hasError}" :style="__style">
    ...
    <div class="col-auto title full-width text-subtitle1 row q-px-xs" v-if="!dense && (__title || $slots.title)" @click="__titleClick" :class="{'cursor-pointer': enableTitleClick}" >
      <div class="col-auto" v-if="resource">
        <q-icon :name="__resourceMeta.icon" :color="__resourceMeta.color" />
      </div>
      <div class="col">
        <slot name="title">
      ...
      <div class="col-auto" v-if="resource">
        <q-icon v-show="__devicedisconnected" name="mdi-lan-disconnect" color="warning" />
        <q-icon v-show="__deviceerror" name="mdi-alert" color="negative" />
      </div>
    </div>

    <div class="col relative-position" style="overflow: hidden;">
      <div v-show="__hasError" class="absolute fit widget-err-content column" style="z-index: 5;">
        <!--<q-btn class="absolute-top-right" flat icon="close" @click="error=null"/>-->
        <div class="col-auto">
          <slot name="error-before"></slot>
        </div>
        <div class="col overflow-auto text-center text-caption">{{ String(error || 'error') }}</div>
        <div class="col-auto">
...
export default {
  ...
  computed: {
    ...
    __widget () {
      ...
      if (typeof widget === 'string') {
        if (this.resource) {
          ...
          if (!widget) {
            errStr = 'widget "' + widgetId + '" not found for the resource ' + this.resource.name()
          ...
          if (!widget) {
            errStr = 'unknown widget type: ' + widgetType
            console.error(errStr)
            this.setError(errStr)
            widget = emptyWidget
          }
    ...
    __devicedisconnected () {
      return this.resource && this.resource.connected && !this.resource.connected()
    },
    __deviceerror () {
      return this.resource && this.resource.attr('error')
    }
  },
